...
@interface APVideoViewController () <UITableViewDataSource,UITableViewDelegate,YTPlayerViewDelegate,UIScrollViewDelegate,UITextViewDelegate>
...
@implementation APVideoViewController
...
- (void)updateData {
    ...
    if (self.loadingData != YES) {
        ...
        [[APServerManager sharedManager]getVideoComment:self.video.owner_id videoid:self.video.videoid count:20 offset:0 onSuccess:^(NSArray *videoCommentArray) {
...
- (void)getCommentsFromServer {
    ...
    if (self.loadingData != YES) {
        ...
        [[APServerManager sharedManager]getVideo:self.video.owner_id videoid:self.video.videoid onSuccess:^(APVideo *video) {
            
            self.video = video;
            
            [self.tableView reloadData];
            
            [[APServerManager sharedManager]getVideoComment:self.video.owner_id videoid:self.video.videoid count:20 offset:[self.commentArray count] onSuccess:^(NSArray *videoCommentArray) {
...
- (void)addLikeToComment:(UIButton *)sender {
    ...
    APCommentsPost *comment = [self.commentArray objectAtIndex:indexPath.row];
    
    
    /*
    if (comment.can_like) {
        [[APServerManager sharedManager]postLikeOnWall:self.video.owner_id inPost:comment.coment_id type:@"video_comment" onSuccess:^(NSDictionary *result) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        APDeatailsVideoCell     *cell = (APDeatailsVideoCell*)[tableView dequeueReusableCellWithIdentifier:identifier];
        
        tableView.separatorInset = UIEdgeInsetsMake(0, 0, 0, 0);
        
        NSDictionary *playerVars = @{ @"playsinline":@1, @"showinfo":@0 };
        
        NSLog(@"self video %@", [self.video.playerURl lastPathComponent] );
        
        if (self.video.playerURl != nil) {
            [cell.youTubeView loadWithVideoId:[self.video.playerURl lastPathComponent]  playerVars:playerVars];
        }
        
        cell.groupNameLabel.text = [[[APServerManager sharedManager] group] name];
        ...
        NSURLRequest *request = [[NSURLRequest alloc]initWithURL:[NSURL URLWithString:[[[APServerManager sharedManager] group] photo_200]]];
        
        __weak APDeatailsVideoCell *weakCell = cell;
        ...
        APPostCell *cell = [tableView dequeueReusableCellWithIdentifier:postCellIdentifier];
        
        if (!cell) {
            cell = [[APPostCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:postCellIdentifier];
        }
        ...
        cell.postDateLabel.text = comment.date;
        
        if ([comment.text isEqualToString:@""]) {
            cell.postTextLabel.text = comment.urlLink;
        } else {
        ...
        [cell.addLikeBtn setTitle:[NSString stringWithFormat:@"%ld",comment.likesCount] forState:UIControlStateNormal];
        [cell.addLikeBtn addTarget:self action:@selector(addLikeToComment:) forControlEvents:UIControlEventTouchUpInside];
        NSURLRequest *request = nil;
        
        if (comment.user != nil) {
            request = [[NSURLRequest alloc]initWithURL:[NSURL URLWithString:comment.user.photo_100]];
            cell.nameSurnameLabel.text = [NSString stringWithFormat:@"%@ %@",comment.user.firstName,comment.user.lastName];
            
        } /*else if (comment.group != nil) {
        ...
        if (comment.photoLink) {
            ...
            APImages *galery = [[APImages alloc]initWithImageArray:comment.photoPhoto startPoint:point];
            galery.tag = 11;
