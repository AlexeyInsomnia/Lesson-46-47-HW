...
@interface APImages () <MHFacebookImageViewerDatasource>
...
- (instancetype) initWithImageArray:(NSArray *)imageArray startPoint:(CGPoint)point {
    ...
    if (self) {
        ...
        for (id Obj in self.images) {
            ...
            if ([Obj isKindOfClass:[APPhoto class]]) {
                
                APPhoto *imageObj = (APPhoto *)Obj;
            ...
            } else if ([Obj isKindOfClass:[APVideo class]]) {
                
                APVideo *imageObj = (APVideo *)Obj;
...
- (void)openVideoAction:(UIButton *)sender {
    ...
    APVideo *video = [self.images objectAtIndex:btn.tag];
...
-  (NSURL*) imageURLAtIndex:(NSInteger)index imageViewer:(MHFacebookImageViewer *)imageViewer {
    ...
    if ([[self.images objectAtIndex:index] isKindOfClass:[APPhoto class]]) {
        APPhoto *photo = [self.images objectAtIndex:index];
        
        return [NSURL URLWithString:photo.photo_604];
        
    } else if ([[self.images objectAtIndex:index] isKindOfClass:[APVideo class]]) {
        
        APVideo *video = [self.images objectAtIndex:index];
